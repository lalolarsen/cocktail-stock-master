...
DECLARE
  ...
  v_ingredient_usage jsonb := '[]'::jsonb;
  v_supabase_url text;
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'payment_summary failed: %', SQLERRM;
    v_payment_summary := '{}'::jsonb;
  END;

  BEGIN
    WITH alc AS (
      SELECT COALESCE(pt.name, 'Sin POS') AS pos_name,
      ...
      FROM sales s LEFT JOIN pos_terminals pt ON pt.id = s.pos_id
      WHERE s.jornada_id = p_jornada_id AND s.is_cancelled = false
      GROUP BY COALESCE(pt.name, 'Sin POS')
    ),
    tk AS (
      SELECT COALESCE(pt.name, 'Sin POS') AS pos_name,
      ...
      FROM ticket_sales ts LEFT JOIN pos_terminals pt ON pt.id = ts.pos_id
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'pos_breakdown failed: %', SQLERRM;
    v_pos_breakdown := '[]'::jsonb;
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'top_products failed: %', SQLERRM;
  ...
  BEGIN
    WITH ings AS (
      SELECT ci.product_id,
             SUM(si.quantity * ci.quantity)::numeric AS qty_used
      FROM sale_items si
      JOIN sales s ON s.id = si.sale_id
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'product_name', p.name,
      'quantity', round(qty_used, 1),
      'unit', CASE WHEN COALESCE(p.capacity_ml,0) > 0 THEN 'ml' ELSE COALESCE(p.unit,'u') END
    ) ORDER BY qty_used DESC), '[]'::jsonb)
    INTO v_ingredient_usage
    FROM (SELECT * FROM ings ORDER BY qty_used DESC LIMIT 15) top
    JOIN products p ON p.id = top.product_id;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'ingredient_usage failed: %', SQLERRM;
