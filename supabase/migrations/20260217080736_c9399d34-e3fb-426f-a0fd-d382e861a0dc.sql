...
BEGIN
  ...
  IF v_missing_pos IS NOT NULL AND array_length(v_missing_pos, 1) > 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Faltan arqueos para ' || array_length(v_missing_pos, 1) || ' POS', 'failing_step', 'missing_pos', 'missing_pos_ids', to_jsonb(v_missing_pos));
  END IF;
  ...
  LOOP
    ...
    -- FIX: use total_amount instead of total
    ...
    IF abs(v_difference) > 0.01 AND (v_closing.notes IS NULL OR trim(v_closing.notes) = '') THEN
      RETURN jsonb_build_object('success', false, 'error', 'POS con diferencia de $' || round(v_difference, 0) || ' requiere notas justificativas', 'failing_step', 'notes_required', 'pos_id', v_closing.pos_id, 'difference', v_difference);
    END IF;

    INSERT INTO jornada_cash_closings (jornada_id, pos_id, venue_id, created_by, opening_cash_amount, cash_sales_total, expected_cash, closing_cash_counted, difference, notes)
    VALUES (p_jornada_id, v_closing.pos_id, v_venue_id, v_user_id, v_opening_cash, v_cash_sales, v_expected, v_closing.closing_cash_counted, v_difference, NULLIF(trim(v_closing.notes), ''));
  END LOOP;
  ...
  VALUES (p_jornada_id, v_venue_id, 'close', v_user_id, 'manual', jsonb_build_object('cash_closings_count', jsonb_array_length(p_cash_closings)));
