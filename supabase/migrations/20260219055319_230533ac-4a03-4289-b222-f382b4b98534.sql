...
-- =====================================================
-- Fix: pickup_redemptions_log venue_id NOT NULL error
-- Strategy: Add safe helper + patch all inserts inside
--           redeem_pickup_token to include venue_id
-- =====================================================

-- 1. Safe helper function (for external callers if needed)
CREATE OR REPLACE FUNCTION public.safe_log_pickup_redemption(
  p_pickup_token_id uuid,
  p_sale_id uuid,
  p_bartender_id uuid,
  p_pos_id text,
  p_result text,
...
BEGIN
  ...
  FROM pickup_tokens
  WHERE id = p_pickup_token_id;

  IF v_venue_id IS NULL THEN
    RAISE EXCEPTION 'pickup_token sin venue_id válido: %', p_pickup_token_id;
  END IF;

  INSERT INTO pickup_redemptions_log (
    pickup_token_id, sale_id, bartender_id, pos_id,
    result, redeemed_at, created_at, venue_id, metadata
  )
  VALUES (
    p_pickup_token_id, p_sale_id, p_bartender_id, p_pos_id::uuid,
    p_result, now(), now(), v_venue_id, p_metadata
  );
END;
$$;

-- 2. Replace redeem_pickup_token with venue_id in every INSERT
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Cannot get venue_id when token not found; use fallback venue
    -- (log will still have bartender info; venue_id is best-effort here)
    DECLARE
  ...
  -- Resolve venue_id once — used in all subsequent log inserts
  ...
  IF v_token_record.status = 'redeemed' THEN
    ...
    VALUES (
      COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
      v_token_record.id, v_token_record.sale_id, 'already_redeemed', v_bar_location_id, v_venue_id,
  ...
  IF v_token_record.status = 'expired' OR (v_token_record.expires_at IS NOT NULL AND v_token_record.expires_at < now()) THEN
    ...
    VALUES (
      COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
      v_token_record.id, v_token_record.sale_id, 'expired', v_bar_location_id, v_venue_id,
  ...
  IF v_token_record.status = 'cancelled' THEN
    ...
    VALUES (
      COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
      v_token_record.id, v_token_record.sale_id, 'cancelled', v_bar_location_id, v_venue_id,
  ...
  IF v_token_record.source_type = 'ticket' AND v_token_record.cover_cocktail_id IS NOT NULL THEN
    ...
    IF NOT FOUND THEN
      ...
      VALUES (
        COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
        v_token_record.id, v_token_record.sale_id, 'not_found', v_bar_location_id, v_venue_id,
    ...
    IF jsonb_array_length(v_missing_items) > 0 THEN
      ...
      VALUES (
        COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
        v_token_record.id, v_token_record.sale_id, 'stock_error', v_bar_location_id, v_venue_id,
    ...
    VALUES (
      COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
      v_token_record.id, v_token_record.sale_id, 'success', v_bar_location_id, v_venue_id,
  ...
  IF v_token_record.payment_status != 'paid' THEN
    ...
    VALUES (
      COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid),
      v_token_record.id, v_token_record.sale_id, 'not_paid', v_bar_location_id, v_venue_id,
...
'DiStock redemption function. venue_id is always derived from pickup_tokens.venue_id — never from frontend state.';

COMMENT ON FUNCTION public.safe_log_pickup_redemption(uuid, uuid, uuid, text, text, jsonb) IS
'Safe helper to log redemptions from external callers. venue_id always sourced from pickup_tokens table.';
