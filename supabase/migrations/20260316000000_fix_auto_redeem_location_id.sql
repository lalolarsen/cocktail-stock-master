-- ══════════════════════════════════════════════════════════════════
-- Fix: auto_redeem_sale_token pre-flight stock check uses wrong column
--
-- The pre-flight check was querying stock_movements.location_id which
-- does not exist (table has from_location_id / to_location_id).
-- The correct table for available stock per location is stock_balances,
-- which is also what consume_stock_fefo uses internally.
-- ══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- FIX: use stock_balances (has location_id) instead of stock_movements
      -- (which has from_location_id/to_location_id, not location_id)
      SELECT COALESCE(sb.quantity, 0) INTO v_available_qty
      FROM stock_balances sb
      WHERE sb.product_id = v_effective_product_id
        AND sb.location_id = p_bar_location_id;

      -- If no row found, available qty is 0
      IF NOT FOUND THEN
        v_available_qty := 0;
      END IF;
