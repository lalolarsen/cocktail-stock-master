...
-- Recreate the FULL version (with mixer overrides + delivered_by_worker_id)
...
BEGIN
  ...
  IF v_token_record.source_type = 'ticket' AND v_token_record.cover_cocktail_id IS NOT NULL THEN
    ...
    IF jsonb_array_length(v_missing_items) > 0 THEN
      INSERT INTO pickup_redemptions_log (bartender_id, pickup_token_id, sale_id, result, pos_id, venue_id, metadata, delivered_by_worker_id)
      VALUES (COALESCE(v_bartender_id, '00000000-0000-0000-0000-000000000000'::uuid), v_token_record.id, v_token_record.sale_id, 'stock_error', v_bar_location_id, v_venue_id, jsonb_build_object('missing', v_missing_items, 'bar_name', v_bar_name, 'consumed', v_consumed_ingredients), p_delivered_by_worker_id);
