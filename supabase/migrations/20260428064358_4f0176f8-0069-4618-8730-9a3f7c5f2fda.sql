-- 1. Add new columns for physical reconciliation confirmation
ALTER TABLE public.jornada_cash_closings
  ADD COLUMN IF NOT EXISTS bartender_name TEXT,
  ADD COLUMN IF NOT EXISTS physical_reconciliation_confirmed BOOLEAN NOT NULL DEFAULT false;

-- 2. Make numeric columns nullable (cuadre is now external)
ALTER TABLE public.jornada_cash_closings
  ALTER COLUMN closing_cash_counted DROP NOT NULL,
  ALTER COLUMN difference DROP NOT NULL;

-- 3. Rewrite close_jornada_manual: checklist-based close
...
BEGIN
  ...
  -- All active cash-register POS must be confirmed
  ...
  -- Validate each entry: confirmed=true and bartender_name not blank
  FOR v_closing IN
  ...
  LOOP
    ...
    -- Compute reference cash data (for audit trail only — does not block close)
    ...
    VALUES (
      ...
      NULL, NULL, NULLIF(trim(v_closing.notes), ''),
