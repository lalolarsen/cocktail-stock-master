CREATE OR REPLACE FUNCTION public.get_monthly_jornadas_summary(p_year integer, p_month integer)
 RETURNS TABLE(jornada_id uuid, numero_jornada integer, nombre text, fecha date, semana_inicio date, hora_apertura time without time zone, hora_cierre time without time zone, estado text, forced_close boolean, requires_review boolean, total_sales numeric, sales_count integer, cancelled_total numeric, cancelled_count integer, alcohol_sales numeric, ticket_sales numeric, cash_sales numeric, card_sales numeric, other_payments numeric, top_sellers jsonb, financial jsonb)
 LANGUAGE plpgsql
...
BEGIN
  ...
  seller_alcohol AS (
    SELECT s.jornada_id AS j_id, s.seller_id AS seller_id, SUM(s.total_amount) AS total, COUNT(*) AS cnt
    FROM public.sales s
    WHERE s.venue_id = v_venue_id
  ...
  seller_tickets AS (
    SELECT t.jornada_id AS j_id, t.sold_by_worker_id AS seller_id, SUM(t.total)::numeric AS total, COUNT(*) AS cnt
    FROM public.ticket_sales t
    WHERE t.venue_id = v_venue_id
      AND t.payment_status = 'paid'
  ...
  sellers_combined AS (
    SELECT u.j_id, u.seller_id, SUM(u.total) AS total, SUM(u.cnt) AS cnt
    FROM (
      SELECT * FROM seller_alcohol
      UNION ALL
      SELECT * FROM seller_tickets
    ) u
    GROUP BY u.j_id, u.seller_id
  ),
  sellers_ranked AS (
    SELECT
      sc.j_id,
      ...
      ROW_NUMBER() OVER (PARTITION BY sc.j_id ORDER BY sc.total DESC) AS rk
    FROM sellers_combined sc
    LEFT JOIN public.profiles p ON p.id = sc.seller_id
  ),
  top_sellers_agg AS (
    SELECT sr.j_id,
      jsonb_agg(jsonb_build_object('name', sr.name, 'total', sr.total, 'count', sr.cnt) ORDER BY sr.total DESC) AS top
    FROM sellers_ranked sr
    WHERE sr.rk <= 3
    GROUP BY sr.j_id
  ),
  fin AS (
    SELECT f.jornada_id AS j_id,
      jsonb_build_object(
  ...
  FROM js
  LEFT JOIN alcohol a ON a.j_id = js.id
  LEFT JOIN tickets t ON t.j_id = js.id
  LEFT JOIN top_sellers_agg ts ON ts.j_id = js.id
  LEFT JOIN fin ON fin.j_id = js.id
