-- 1. Add the inventory_freeze_mode flag to the master table
INSERT INTO feature_flags_master (key, name, description, default_enabled)
VALUES ('inventory_freeze_mode', 'Modo Congelamiento Inventario', 'Marcha blanca: ventas normales sin descuento de stock', false)
ON CONFLICT (key) DO NOTHING;

-- 2. Helper function to check if inventory is frozen for a venue
...
-- 3. Modify auto_redeem_sale_token to skip stock deduction when frozen
...
BEGIN
  ...
  v_frozen := is_inventory_frozen(v_token_record.venue_id);
  ...
  VALUES (p_seller_id, v_token_record.id, p_sale_id, 'success'::redemption_result, p_bar_location_id,
    v_token_record.venue_id, jsonb_build_object(
      'auto_redeem', true, 
      'bar_name', v_bar_name, 
...
-- 4. Modify redeem_pickup_token to skip stock deduction when frozen
CREATE OR REPLACE FUNCTION public.redeem_pickup_token(
  ...
  p_delivered_by_worker_id uuid DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_token_record.source_type = 'ticket' THEN
    ...
    IF v_token_record.cover_cocktail_id IS NOT NULL AND v_bar_location_id IS NOT NULL AND NOT COALESCE(v_venue_is_demo, false) AND NOT v_frozen THEN
    ...
    VALUES (v_token_record.id, v_bartender_id, 'success',
            jsonb_build_object('deliver', v_deliver, 'bar', v_bar_name, 'consumed', v_consumed_ingredients, 'inventory_frozen', v_frozen),
  ...
  ELSE
    -- ========== SALE TOKEN ==========
    IF v_bar_location_id IS NOT NULL AND NOT COALESCE(v_venue_is_demo, false) AND NOT v_frozen THEN
      FOR v_item IN
    ...
    INSERT INTO pickup_redemptions_log (pickup_token_id, bartender_id, result, metadata, pos_id, jornada_id, venue_id, delivered_by_worker_id)
    VALUES (v_token_record.id, v_bartender_id, 'success',
            jsonb_build_object('deliver', v_deliver, 'bar', v_bar_name, 'sale_number', v_token_record.sale_number, 'consumed', v_consumed_ingredients, 'inventory_frozen', v_frozen),
            p_bartender_bar_id, v_active_jornada_id, v_venue_id, p_delivered_by_worker_id);
