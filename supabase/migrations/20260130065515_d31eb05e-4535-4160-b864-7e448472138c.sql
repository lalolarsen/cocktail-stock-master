...
-- Update redeem_pickup_token RPC to accept mixer overrides
-- This allows bartenders to select which mixer to use for drinks with interchangeable mixer slots

CREATE OR REPLACE FUNCTION redeem_pickup_token(
  p_token text,
  p_bartender_bar_id uuid DEFAULT NULL,
  p_mixer_overrides jsonb DEFAULT NULL  -- Format: [{"slot_index": 1, "product_id": "uuid"}]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ALREADY_REDEEMED: Return immediately with NO stock changes
  ...
  -- STEP 6: Process COVER TOKEN (from ticket sale)
  IF v_token_record.source_type = 'ticket' AND v_token_record.cover_cocktail_id IS NOT NULL THEN
    ...
    -- DEDUCT STOCK with mixer overrides
  ...
  -- Process each sale item with mixer overrides
  FOR v_item IN
...
-- Create a function to check if a token requires mixer selection before redemption
CREATE OR REPLACE FUNCTION check_token_mixer_requirements(p_token text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  -- Find the token
  SELECT pt.*, s.sale_id as parent_sale_id
  INTO v_token_record
  FROM pickup_tokens pt
  LEFT JOIN sales s ON s.id = pt.sale_id
  WHERE pt.token = p_token;
