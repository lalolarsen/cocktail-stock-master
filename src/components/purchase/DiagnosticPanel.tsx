...
export function DiagnosticPanel({ rawExtraction, computedLines, className }: DiagnosticPanelProps) {
            ...
            <div className="space-y-2">
              ...
              <pre className="bg-muted p-3 rounded-lg text-xs overflow-auto max-h-64 font-mono">
                {JSON.stringify(computedLines.map(l => ({
                  ...
                  real: l.real_units,
                  gross: l.gross_line,
                  disc: l.discount_amount,
                  tax: l.taxes_excluded_for_cost,
                  net: l.net_line_for_cost,
                  cost: l.net_unit_cost,
                  reasons: l.reasons,
                })), null, 2)}
              </pre>
            </div>
            ...
            <div className="bg-muted/50 p-3 rounded-lg space-y-1 text-xs">
              ...
              <div className="grid grid-cols-2 gap-1">
                ...
                <span className="text-right font-mono">
                  ${computedLines.reduce((s, l) => s + l.gross_line, 0).toLocaleString("es-CL")}
                </span>
                <span>Total Descuentos:</span>
                <span className="text-right font-mono text-red-600">
                  -${computedLines.reduce((s, l) => s + l.discount_amount, 0).toLocaleString("es-CL")}
                </span>
                <span>Total Impuestos:</span>
                <span className="text-right font-mono text-purple-600">
                  -${computedLines.reduce((s, l) => s + l.taxes_excluded_for_cost, 0).toLocaleString("es-CL")}
                </span>
                <span className="font-medium">Total Neto:</span>
                <span className="text-right font-mono font-medium text-green-700">
                  ${computedLines.reduce((s, l) => s + l.net_line_for_cost, 0).toLocaleString("es-CL")}
                </span>
              </div>
            </div>
          </CardContent>
